...
output <- foreach(i = 1:niter, .packages = c("dplyr","tidyr","fda","LaplacesDemon","fossil","combinat")) %dopar% {
  ...
  clustering       <- c(rd_result, ard_result, rd_result0, ard_result0)
  
  PART0result <- list(Bayesian = Part0_for_L, Frequentist = Part0_for_L_freq, Clustering = clustering)
  # -------------------------------------------------------------------------------------------------
  #
  ###################################################################################################
  PART0result
...
#
###################################################################################################
# save(output, file="PART0result500.RData")
