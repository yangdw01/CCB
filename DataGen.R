...
# Data generation 
# -------------------------------------------------------------------------------------------------
DataGen <- function(isPART0, seednum, delta=NULL, scenario=NULL){
  ...
  if(isPART0 == T)
  {
    ...
    tmp_param_dirichlet <- c(5/10, 4/10, 1/10, 0)*400 # 200 #
    ...
    # data generation for part 1 and 2
    # parameters for dirichlet distribution
    if(scenario == 1)
    {
      tmp_param_dirichlet1 <- c(5/10, 4/10, 1/10, 0)*400
      tmp_param_dirichlet2 <- c(5/10, 4/10-delta, 1/10, delta)*400
    }else if(scenario == 2)
    {
      tmp_param_dirichlet1 <- c(5.5/10, 4.5/10-delta, delta, 0)*400
      tmp_param_dirichlet2 <- c(5.5/10, 4.5/10, 0, 0)*400
    }else if(scenario == 3)
    {
      tmp_param_dirichlet1 <- c(5/10, 4/10, 1/10, 0)*400
      tmp_param_dirichlet2 <- c(5/10, 4/10-delta, 1/10+delta, 0)*400
    }else if(scenario == 4)
    {
      tmp_param_dirichlet1 <- c(5.5/10, 4.5/10-delta, delta, 0)*400
      tmp_param_dirichlet2 <- c(5.5/10, 4.5/10-delta, 0, delta)*400
    }
...
# library(plotly)
# 
# plot_data <- tibble(Cluster=NULL, v1=NULL, v2=NULL, y=NULL, mean=NULL)
# sig <- 1
# for(i in 1:4)
# {
#   tmp_coef <- TRUE_COEF[i,] + rnorm(length(TRUE_COEF[i,]), 0, 0.1)
#   tmp_mean <- c(cbind(sin(c(t1)), cos(c(t2)), c(t1)*c(t2)) %*% tmp_coef)
#   tmp_y <- tmp_mean + rnorm(length(t1), 0, sig)
#   tmp_data <- tibble(Cluster=i, v1=c(t1), v2=c(t2), y=tmp_y, mean=tmp_mean)
#   plot_data <- plot_data %>% bind_rows(tmp_data)
# }
# plot_data <- plot_data %>% mutate(Cluster = as.factor(Cluster))
# 
# cluster <- 4
# tmp_cluster_data1 <- plot_data %>% filter( Cluster == cluster ) %>% select(-Cluster, -y)
# tmp_cluster_data2 <- plot_data %>% filter( Cluster == cluster ) %>% select(-Cluster, -mean)
# 
# tmp_cluster_data1 <- tmp_cluster_data1 %>% pivot_wider(names_from = v2, values_from = mean) %>% select(-v1) %>% as.matrix
# #tmp_cluster_data2 <- tmp_cluster_data2 %>% pivot_wider(names_from = v2, values_from = y) %>% select(-v1) %>% as.matrix
# 
# titlename <- c("<b>(a)<b>", "<b>(b)<b>", "<b>(c)<b>", "<b>(d)<b>")
# plot_ly() %>% add_surface(x=t1, y=t2, z=~tmp_cluster_data1,
#                           zmin=-5,zmax=5,
#                           colorbar=list(title='', tickfont=list(size=20), x=1, y=0.8)) %>%
#   add_markers( data=tmp_cluster_data2, x=~v1, y=~v2, z=~y, marker=list(size=2), alpha=0.9) %>%
...
#          title = titlename[cluster], titlefont = list(size = 50, face="bold"),
#          margin = list(t=100))
# # -------------------------------------------------------------------------------------------------
# #
# ###################################################################################################