...
# parallel computing
output <- foreach(i = 1:niter, .packages = c("dplyr","tidyr","fda","LaplacesDemon","fossil","combinat")) %dopar% {
  ...
  seed_num <- i
  delta <- delta
  DATA <- DataGen_sc2(isPART0=isPART0, seednum=seed_num, delta=delta)
  # -------------------------------------------------------------------------------------------------
  #
  ###################################################################################################
  ...
  # save result
  Part0_for_L      <- list(centerline = Phase1_piks,          sigma=Phase1_confints_sigma,      PS=multinomial_result)
  Part0_for_L_freq <- list(centerline = Phase1_cluster_props, sigma=Phase1_cluster_props_sigma, PS=freq_PS)
  PART0result <- list(Bayesian = Part0_for_L, Frequentist = Part0_for_L_freq)
  # -------------------------------------------------------------------------------------------------
  #
  ###################################################################################################
  PART0result
...
#
###################################################################################################
save(output, file=paste("PART0result_", niter, "_scenario2_", delta, ".RData", sep=""))
