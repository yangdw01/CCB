...
####      JQT         by DY
####
####      (Simulation 2) - PART2. ARL1 & Rand index
####
####
###################################################################################################
###################################################################################################
...
# Code explanations :
# 
#   Obtain the result for ARL1 and clustering (rand index)
#
###################################################################################################
...
# create the cluster
myCluster <- makeCluster(28)

# register the cluster with the foreach package
registerDoParallel(myCluster)

# parallel computing
output <- foreach(i = 1:niter, # .export = c("scenario","delta"), 
                  .packages = c("dplyr","tidyr","fda","LaplacesDemon","fossil","combinat")) %dopar% {
  ...
  Step1_result_Kmeans <- kmeans(Step1_result, centers=3, iter.max=10000, nstart = 10)
  ...
  for(w in 1:(Phase2_IC_SS + Phase2_OOC_SS)){
  ...
  Step1_result_Kmeans <- kmeans(Step1_result[Phase1_idx,], centers=3, iter.max=10000, nstart=10) # Kmeans_custom(Step1_result)
  Phase1_cluster_props <- (Step1_result_Kmeans$cluster %>% table())/(Subgroup_SS * Phase1_SS)
  ...
  rd <- rand.index(est_clusters, true_clusters)
  ard <- adj.rand.index(est_clusters, true_clusters)
  
  # save result
  Part2_for_ARL1      <- list(centerline = Phase1_piks,          sigma=Phase1_confints_sigma,      PS=multinomial_result)
  Part2_for_ARL1_freq <- list(centerline = Phase1_cluster_props, sigma=Phase1_cluster_props_sigma, PS=freq_PS)
  tmp_clustering_result <- c(rd_result, ard_result, rd, ard)
  PART2result <- list(Bayesian = Part2_for_ARL1, Frequentist = Part2_for_ARL1_freq, Clustering = tmp_clustering_result)
  # -------------------------------------------------------------------------------------------------
  #
  ###################################################################################################
  PART2result
...
#
###################################################################################################
save(output, file=paste("PART2_scenario", scenario, "delta", delta, "_result.RData", sep=""))
...
# source("DataGen.R")
# scenario <- 2 # 1 # 2 # 3 #
# delta <- 0.1 # 0.05 # 0.1 # 0.15 # 0.2 #
# L <- 3.02
# L2 <- 8.16
# load(file=paste("PART2_scenario", scenario, "delta", delta, "_result.RData", sep=""))
# 
# # ARL1
# ARL1_result <- NULL
# for(i in seq(output))
# {
#   tmp_LCL <- output[[i]]$Bayesian[[1]] - L * output[[i]]$Bayesian[[2]]
#   tmp_UCL <- output[[i]]$Bayesian[[1]] + L * output[[i]]$Bayesian[[2]]
#   tmp_alarm_chk <- (matrix(tmp_LCL, 
#                            nrow=nrow(output[[i]]$Bayesian[[3]]), 
#                            ncol=length(output[[i]]$Bayesian[[1]]), byrow=T) > output[[i]]$Bayesian[[3]]) | 
#     (matrix(tmp_UCL, nrow=nrow(output[[i]]$Bayesian[[3]]), 
#             ncol=length(output[[i]]$Bayesian[[1]]), 
#             byrow=T) < output[[i]]$Bayesian[[3]])
#   tmp_RL1 <- tmp_alarm_chk[-(1:Phase2_IC_SS),] %>% apply(2, function(x){ ifelse(length(which(x))>0, which(x)[1], Phase2_OOC_SS) }) %>% min()
#   ARL1_result <- c(ARL1_result, tmp_RL1)
# }
# 
# ARL1_result2 <- NULL
# for(i in seq(output))
# {
#   tmp_LCL <- output[[i]]$Frequentist[[1]] - L2 * output[[i]]$Frequentist[[2]]
#   tmp_UCL <- output[[i]]$Frequentist[[1]] + L2 * output[[i]]$Frequentist[[2]]
#   tmp_alarm_chk <- (matrix(tmp_LCL, 
#                            nrow=nrow(output[[i]]$Frequentist[[3]]), 
#                            ncol=length(output[[i]]$Frequentist[[1]]), byrow=T) > output[[i]]$Frequentist[[3]]) | 
#     (matrix(tmp_UCL, nrow=nrow(output[[i]]$Frequentist[[3]]), 
#             ncol=length(output[[i]]$Frequentist[[1]]), byrow=T) < output[[i]]$Frequentist[[3]])
#   tmp_RL1 <- tmp_alarm_chk[-(1:Phase2_IC_SS),] %>% apply(2, function(x){ ifelse(length(which(x))>0, which(x)[1], Phase2_OOC_SS) }) %>% min()
#   ARL1_result2 <- c(ARL1_result2, tmp_RL1)
# }
# 
# ARL1_result %>% mean()
# ARL1_result2 %>% mean()
...
# Frequentist_clustering <- sapply(1:length(output), function(x){ output[[x]]$Clustering[[3]] })
# 
# Bayesian_clustering %>% mean()
# Frequentist_clustering %>% mean()